...
const ProductCard= ({product}) => {
    ...
    return product && (
        <div className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-56 max-w-56 w-full">
            <div onClick={()=> navigate(`/product/${product._id}`)} className="group cursor-pointer flex items-center justify-center px-2">
                <img className="group-hover:scale-105 transition max-w-26 md:max-w-36" src={product.image[0]} alt={product.name} />
            </div>
            <div className="text-gray-500/60 text-sm">
                <p>{product.category}</p>
                <p onClick={()=> navigate(`/product/${product._id}`)} className="text-gray-700 font-medium text-lg truncate w-full cursor-pointer">{product.name}</p>
